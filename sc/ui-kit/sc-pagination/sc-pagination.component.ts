...
export class ScPaginationComponent implements OnChanges {
  ...
  /**
   * Если в поле ввода страницы внесены корректные данные, то переходим на нужную страницу.
   * @param e
   */
  private validateInput(e) {
    let val = +e.target.value;
    val && (val <= this.getTotalPages()) && this.setCurrentPageIndex(val);
  }

  /**
   * Проверяем введенные значения на keyCode
   * @param e
   */
  private onlyNumbers(e) {
    if (
      ...
      [46, 8, 9, 27, 13, 110, 190, 39, 37].indexOf(e.keyCode) === -1
    ) {
      e.preventDefault();
    }
  }
