...
import {ScWindowListComponent, ResultTemplateContext} from './../sc-window-list/sc-window-list.component';
import {PopupService} from '@ng-bootstrap/ng-bootstrap/util/popup';
import {positionElements} from '@ng-bootstrap/ng-bootstrap/util/positioning';
import {toString, isDefined} from '@ng-bootstrap/ng-bootstrap/util/util';
import {ScTypeaheadConfig} from './sc-typeahead-config';
...
const SC_TYPEAHEAD_VALUE_ACCESSOR = {
    provide: NG_VALUE_ACCESSOR,
    useExisting: forwardRef(() => ScTypeaheadDirective),
    multi: true
};
 ...
 */
export interface ScTypeaheadSelectItemEvent {
    /**
...
/**
 * ScTypeahead directive provides a simple way of creating powerful typeaheads from any text input
 */
@Directive({
    selector: 'input[scTypeahead]',
    host: {
        '(blur)': 'handleBlur()',
    ...
    providers: [SC_TYPEAHEAD_VALUE_ACCESSOR]
})
export class ScTypeaheadDirective implements ControlValueAccessor, OnInit, OnDestroy {
    private _popupService: PopupService<ScWindowListComponent>;
    private _subscription: Subscription;
    private _userInput: string;
    private _valueChanges: Observable<string>;
    private _resubscribeTypeahead: BehaviorSubject<any>;
    private _windowRef: ComponentRef<ScWindowListComponent>;
     ...
     */
    @Input() scTypeahead: (text: Observable<string>) => Observable<any[]>;
    ...
    /**
     * An event emitted when a match is selected. Event payload is of type scTypeaheadSelectItemEvent.
     */
    @Output() selectItem = new EventEmitter<ScTypeaheadSelectItemEvent>();

    activeDescendant: string;
    popupId = `sc-typeahead-${nextWindowId++}`;
    ...
    constructor(
        ...
        private _injector: Injector, componentFactoryResolver: ComponentFactoryResolver, config: ScTypeaheadConfig,
        ngZone: NgZone
    ) {
        ...
        this._popupService = new PopupService<ScWindowListComponent>(
            ScWindowListComponent, _injector, _viewContainerRef, _renderer, componentFactoryResolver);
    ...
    ngOnInit(): void {
        ...
        const results$ = letProto.call(inputValues$, this.scTypeahead);
